...
if [ ! -d ./vtenv/src/paramiko ];then
    vtenv/bin/pip install -e git+http://github.com/bearstech/paramiko.git#egg=paramiko || echo "Please install python-dev and retry"
else
...
if [ ! -d ./vtenv/src/fabric ];then
    vtenv/bin/pip install -e git+http://github.com/bearstech/fabric.git@ssh-agent#egg=fabric
else
    cd vtenv/src/fabric
